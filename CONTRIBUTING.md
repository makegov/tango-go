...
`.pre-commit-config.yaml` ships hooks that mirror what CI runs. We use [`uv`](https://docs.astral.sh/uv/) to manage [`pre-commit`](https://pre-commit.com/) (same toolchain as the rest of MakeGov's Python tooling):

```bash
brew install uv                  # if you don't already have it
make hooks                       # uv tool install pre-commit + pre-commit install (commit + push hooks)
```
...
Hooks are entirely opt-in — skip `make hooks` and your commits/pushes are unaffected. CI still catches everything.
